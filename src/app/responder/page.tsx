    ...
    >
      <div className="space-y-8 sm:space-y-10">
        ...
        <div className="grid gap-4 sm:gap-6 grid-cols-1 xs:grid-cols-2 sm:grid-cols-3">
          <StatCard 
            title="Active Alerts" 
            value={reports.filter(r => r.severity === 'CRITICAL' || r.severity === 'HIGH').length.toString()} 
        ...
        <div className="grid gap-8 lg:gap-10 grid-cols-1 lg:grid-cols-[1.2fr_2fr]">
          ...
          <div className="space-y-4 order-1 lg:order-2">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-text-primary">Active Incidents (from Citizen Reports)</h2>
            ...
            <div className="space-y-4 sm:space-y-6">
              {loading ? (
                <div className="p-12 text-center text-text-secondary">Loading reports...</div>
              ) : reports.length === 0 ? (
                <div className="p-12 text-center text-text-secondary">No citizen reports found.</div>
              ) : (
                reports.map((report) => (
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4">
        <div className="space-y-4 flex-1">
          <div className="flex flex-wrap gap-2">
            <span className={`rounded-full ${getSeverityColor(report.severity)} px-3 py-1 text-[8px] sm:text-[9px] font-bold text-white uppercase tracking-wider`}>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-x-6 sm:gap-x-8 gap-y-3 text-[10px] sm:text-[11px] text-[#64748B] font-bold">
            <div className="flex items-center gap-2">
              ...
              <span>{report.landmark || report.address || 'Unknown Location'}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="h-3 w-3" />

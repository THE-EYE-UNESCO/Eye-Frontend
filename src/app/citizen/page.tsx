...
const alerts = [
  { label: "Flood Alert", level: "High", location: "Nyabihu - Rwanda", color: "bg-red-500" },
  { label: "Landslide Warning", level: "Critical", location: "Gakenke - Rwanda", color: "bg-orange-500" },
  { label: "Wildfire Detection", level: "Medium", location: "Musanze - Rwanda", color: "bg-yellow-400" },
];

const stories = [
  {
    title: "Community Comes Together In Crisis",
    subtitle: "Neighbors Helping Neighbors – The True Spirit Of Our Community Shines.",
    image: "/sample-crisis.jpg",
  },
  {
    title: "Volunteers Coordinate Rapid Response",
    subtitle: "Local teams organize supplies and support within minutes of alerts.",
    image: "/sample-crisis.jpg",
  },
];

const tableRows = [
  { crisis: "Floods", location: "Kigali", date: "12.08.2019 - 12:53 PM", status: "Low", color: "bg-emerald-400" },
  { crisis: "Floods", location: "Kigali", date: "12.08.2019 - 12:53 PM", status: "Medium", color: "bg-amber-400" },
  { crisis: "Floods", location: "Kigali", date: "12.08.2019 - 12:53 PM", status: "Critical", color: "bg-red-500" },
  { crisis: "Floods", location: "Kigali", date: "12.08.2019 - 12:53 PM", status: "High", color: "bg-orange-500" },
];
    ...
    >
      <div className="space-y-6">
        <div className="overflow-hidden rounded-3xl bg-white shadow-md">
              ...
              <div className="space-y-3 px-6 py-4">
                ...
                <p className="text-xs text-slate-500">
                  Fire crews working around the clock have successfully established firebreaks.
                  Evacuation orders remain in effect for zone MN-65.
                </p>
              </div>

              {/* Alert tags row */}
              <div className="flex flex-wrap gap-3 border-t border-slate-100 bg-slate-50 px-6 py-3 text-xs">
                {alerts.map((alert) => (
                  <div
                    key={alert.label}
                  ...
                  >
                    <div className="space-y-0.5">
                      <p className="text-[11px] font-medium text-slate-600">{alert.location}</p>
                      <p className="text-[11px] text-slate-400">{alert.label}</p>
                    </div>
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold text-white ${alert.color}`}
                    >
                      {alert.level}
                    </span>
                  </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,2fr)_minmax(260px,0.9fr)]">
          ...
          <div className="space-y-4">
            <div className="rounded-3xl bg-[#0b1020] px-5 py-4 text-slate-50 shadow-md">
              ...
              <div className="mt-4 space-y-2 text-xs">
                <ActionButton icon={<MessageCircle className="h-4 w-4" />} label="Report Incident" primary />
                <ActionButton icon={<Phone className="h-4 w-4" />} label="Emergency Contacts" />
                <ActionButton icon={<Bell className="h-4 w-4" />} label="IVR" />
              </div>
            </div>

            <div className="rounded-3xl bg-white px-5 py-4 text-xs text-slate-700 shadow-md">
              <p className="text-sm font-semibold text-slate-900">Safety Guidance</p>
          ...
          <div className="rounded-3xl bg-white px-5 py-4 text-xs text-slate-700 shadow-md">
            <p className="text-sm font-semibold text-slate-900">Crisis Breakdown</p>
                ...
                <tbody>
                  {tableRows.map((row, idx) => (
                    <tr key={idx} className="rounded-xl bg-slate-50">
                      <td className="rounded-l-xl px-3 py-3 text-slate-700">{row.crisis}</td>
                      <td className="px-3 py-3 text-slate-500">{row.location}</td>
                      <td className="px-3 py-3 text-slate-500">{row.date}</td>
                      <td className="rounded-r-xl px-3 py-3">
                        <span
                          className={`inline-flex items-center rounded-full px-3 py-1 text-[10px] font-semibold text-white ${row.color}`}
                        >
                          {row.status}
                        </span>
                      </td>
